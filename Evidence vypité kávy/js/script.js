    ...
    getData(url).then(data => {
        ...
        for (let key in data){
            if (data.hasOwnProperty(key)) {
                const dataInfo = `<label><input type="radio" name="user" value="${data[key].ID}"> ${data[key].name}</label><br>`;
...
function monthlySummary(month){
    const url = "http://ajax1.lmsoft.cz/procedure.php?cmd=getSummaryOfDrinks&month=" + month
        ...
        getData(url).then(data => {
            const consumptionDiv = document.getElementById('monthConsumption');
            consumptionDiv.innerHTML = '<h1>Drink consumption for this month:</h1> <br>';
            consumptionDiv.innerHTML += '<ul>';
            for (let drinks in data){
                    const dataInfo = `<li><bold>Piti:</bold> ${data[drinks][0]} | <bold>Mnozstvi:</bold> ${data[drinks][1]} | <bold>Jmeno:</bold> ${data[drinks][2]}</li>`;
                    consumptionDiv.innerHTML += dataInfo;
            }       
